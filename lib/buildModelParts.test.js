...
test("groups every animated extra top-level object into a single hidden Detail group", () => {
  const { root } = makeParts();
  ...
  const animations = [makeClip("Plane"), makeClip("Plane.001")];

  const result = buildModelParts(root, new LineMaterial(), animations);
...
test("keeps each detail child positioned exactly as authored relative to Solid after grouping", () => {
  ...
  buildModelParts(root, new LineMaterial(), [makeClip("Plane")]);
  root.updateMatrixWorld(true);
...
test("keeps a single object literally named 'Edges2' even with no authored animation", () => {
  const { root } = makeParts();
...
test("removes an unanimated, unrecognized leftover object from the scene entirely", () => {
  // e.g. a Blender-default "Cube" left in by accident: not Solid/Edges,
  // not named "Edges2", and no animation targets it - not something an
  // admin meant to show, so it's deleted outright, not just hidden.
  const { root, solidNode, edgesNode } = makeParts();
  const leftover = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1));
  leftover.name = "Cube";
  root.add(leftover);
  ...
  assert.equal(result.detail, null);
  assert.ok(!root.children.includes(leftover));
  assert.deepEqual(root.children, [solidNode, edgesNode]);
});

test("a deleted leftover never skews the returned size used to normalize the model's scale", () => {
  // `size` feeds AR_MODEL_SIZE_MARKER_WIDTHS scale normalization - an
  // oversized leftover must not be measured before it's deleted, or every
  // model with one would render too small once normalized.
  const { root } = makeParts();
  const hugeLeftover = new THREE.Mesh(new THREE.BoxGeometry(100, 100, 100));
  hugeLeftover.name = "Cube";
  root.add(hugeLeftover);

  const result = buildModelParts(root, new LineMaterial());

  assert.ok(result.size.x < 2);
  assert.ok(result.size.y < 2);
  assert.ok(result.size.z < 2);
});

test("removes only the unrecognized leftover while keeping legitimate detail content", () => {
  const { root } = makeParts();
  const plane = new THREE.Mesh(new THREE.PlaneGeometry(1, 1));
  plane.name = "Plane";
  const leftover = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1));
  leftover.name = "Cube";
  root.add(plane, leftover);

  const result = buildModelParts(root, new LineMaterial(), [makeClip("Plane")]);

  assert.ok(result.detail);
  assert.equal(result.detail.children.length, 1);
  assert.equal(result.detail.children[0], plane);
  assert.ok(!root.children.includes(leftover));
});
